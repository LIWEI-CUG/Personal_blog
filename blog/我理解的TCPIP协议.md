...
3.GET方式提交数据会带来安全问题，比如一个登录页面，通过GET方式提交数据时，用户名和密码出现在URL上，如果页面可以被缓存或者其他人可以访问这台机器，就可以存历史记录获得该用户的账号和密码。
```
...
如果说ARP协议是用来将IP地址转换为MAT地址，那么DNS协议就是将域名转换为IP地址，当然它也可以将IP地址转换为对应的域名地址，域名服务是基于UDP实现的，服务器的端口号为53。
...
- 递归查询：主机向本地域名服务器发出一次查询请求，静待最终的查询结果。如果本地域名服务器无法解析，自己则会以DNS客户机的身份向更高级的域名服务器查询，知道得到目标IP告诉主机。
