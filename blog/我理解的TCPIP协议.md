...
![](D:\github\Personal_blog\picture\OSI 7层协议.png)

![](D:\github\Personal_blog\picture\各层协议.png)

OSI 五层模型是结合七层模型和TCP/IP模型略加改进的，是学习计算机网络原理时采取的一种折衷的办法，在本篇博客中，重点关注的是TCP/IP的上面三层，即应用层，传输层和网络层。
...
![](D:\github\Personal_blog\picture\数据处理流程.png)
...
比如客户端浏览器请求浏览页面www.baidu.com，其实这是一个默认路径，平常默认会省略协议名，端口号，访问主页时，路径也将被省略，所以它的完整写法应该是http://www.baidu.com:80/index.html。这就是我们常说的统一资源定位符（URL），用来定位我们访问资源在服务器上的位置。对于这个URL来说：
...
HTTP请求报文由请求行，请求头和请求体三部分构成。

- 请求行由请求方法，URL地址，协议名称和版本号三块组成。
- 请求头：用来说明服务器要是用的附加信息，包含若干个属性，格式均为“属性名：属性值”，服务端由此获得客户端的信息。
...
![](D:\github\Personal_blog\picture\HTTP请求报文结构.png)

![](D:\github\Personal_blog\picture\HTTP请求报文.png)

##### 1.3 HTTP响应报文

HTTP响应报文也有三部分组成，分别是响应行，响应头，响应体。

- 响应行：报文协议及版本，状态码及描述
- 响应头：跟请求头一样，由属性组成，说明客户端要是用的一些附加信息
- 响应体：是服务器返回给客户端的文本信息

![](D:\github\Personal_blog\picture\HTTP响应报文.png)
...
![](D:\github\Personal_blog\picture\域名服务器的分类.png)

##### 2.2 迭代查询和递归查询

- 递归查询：主机向本地域名服务器发出一次查询请求，静待最终的查询结果。如果本地域名服务器无法解析，自己则会以DNS客户机的身份向更高级的域名服务器查询，直到得到目标IP，告诉主机。
- 迭代查询：本地域名服务器向更域名服务器查询，根域名服务器告诉它下一步到哪里去查询，然后它在根据根域名服务器的指示进行下一步查询，以此类推，每次本地域名服务器都是以客户机的身份去各个服务器查询的。

通俗地说，递归就是把一件事情交给别人，如果事情没有办完，哪怕已经办了很多，都不要把结果告诉我，我要的是你的最终结果，而不是中间结果；如果你没办完，请你找别人办完，把最后结果告诉我就好了。迭代则是我交给你一件事，你能办多少就告诉我你办了多少，然后剩下的事情我按照你的指示继续去办，直到查到最终的结果。
...
- UDP是不可靠的数据报协议，只负责发送数据（可以控制发送消息的大小），但不能保证消息一定会到达接受方，UDP是没有重发机制的，这一机制由应用程序自己根据需要进行定义。
...
![](D:\github\Personal_blog\picture\通过端口号识别应用.png)
...
![](D:\github\Personal_blog\picture\应用程序通信.png)

![](D:\github\Personal_blog\picture\数据报内容.png)
...
- UDP是一种“将应用程序发来的数据在收到的那一刻，立即按照原样发送到网络上”的机制，即使网络中有拥塞的情况，UDP也无法进行拥塞控制来避免拥塞
...
![](D:\github\Personal_blog\picture\单播广播多播.png)
...
![](D:\github\Personal_blog\picture\路由控制表与IP协议.png)
...
![](D:\github\Personal_blog\picture\IPV6地址.png)
