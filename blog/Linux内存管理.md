...
一个可执行的程序（进程）在存储时（为调入内存）分为代码段，数据区和未初始化数据区。

代码段：存放CPU执行的机器指令的一块内存区域，这部分区域的大小在程序运行前就已经确定，并且该区域属于只读，在代码段中可能包含一些只读的常熟变量。
...
Linux利用分页管理基址来更加有效的利用物理内存，当创建一个进程时，仅仅把当前进程的一小部分装入到内存当中，需要访问其余部分时，处理器产生一个页故障，由缺页中断服务程序根据虚拟地址和出错码把需要的页换入内存。随着可执行映像的运行和换入，会导致内存紧张，这时Linux核心就必须调用kswap守护进程释放一部分物理内存，kswap在系统启动时由Init进程建立，在系统的运行过程中，他被定期唤醒，检查内存中空闲的内存是否很少，如果是，则释放一部分内存，或者将一些页面换出到对换空间，然后继续休眠。
